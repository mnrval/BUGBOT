let handler = async (m, { conn }) => {
  ...
  if (typeof werewolf.players == "undefined" || Object.keys(werewolf.players).length == 0) {
    conn.reply(m.chat, "Tidak ada pemain dalam game ini", m)
    return
  }
  ...
  conn.reply(m.chat, `Ada ${players.length} pemain dalam game ini, termasuk ${villagerCount} penduduk desa dan ${wolfCount} manusia serigala:\n\n${playerList}`, m) // mengubah format pesan agar lebih mudah dibaca
}
