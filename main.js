...
var authFolder = storeSystem.fixFileName(`${Helper.opts._[0] || ''}SessionsRifal`)
...
function purgeSession() {
    let prekey = [];
    const directorio = readdirSync('./SessionsRifal');
    const filesFolderPreKeys = directorio.filter((file) => {
        return file.startsWith('pre-key-');
    });
    prekey = [...prekey, ...filesFolderPreKeys];
    filesFolderPreKeys.forEach((files) => {
        unlinkSync(`./SessionsRifal/${files}`);
    });
}
    const directories = ['./SessionsRifal/'];
...
async function connectionUpdate(update) {
  ...
  if (connection === "open") {
       ...
       const infoMsg = `Halo Kak ${global.author},\n\nBot Berhasil Terhubung, Silahkan Kirim Perintah Kepada @${global.namabot} dengan mengetik *.menu* ya kak!.\nTerimakasih🙏😊`;
       conn.sendMessage(nomorown + "@s.whatsapp.net", {
            text: infoMsg,
       mentions: ["6283853305641@s.whatsapp.net", jid]
       }, {
          quoted: global.fakes, 
...
global.reloadHandler = async function(restatConn) {
    ...
    conn.welcome = `${emoji.welcome} Hallo @user\n\n   *W E L C O M E*\n⫹⫺ Di grup @subject\n\n⫹⫺ Jangan Lupa Baca *DESKRIPSI*\n\n_➡︎ https://youtube.com/@RifalModzYT_`;
    conn.bye = `   *G O O D B Y E*\n${emoji.bye} Sampai jumpa @user, semoga amalmu tidak diterima oleh malaikat🗿\n\n_➡︎ https://youtube.com/@RifalModzYT_`;
    conn.spromote = `*${emoji.promote} @user* sekarang anda telah menjadi admin!\n\n_➡︎ https://youtube.com/@RifalModzYT_`;
    conn.sdemote = `*${emoji.demote} @user* sekarang anda lagi menjadi admin!\n\n_➡︎ https://youtube.com/@RifalModzYT_`;
